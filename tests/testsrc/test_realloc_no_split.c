...
main(int argc, char **argv)
{
    ...
    // Allocate memory
    int *mem1 = (int *) mallocing(64, print_status, false);
    printf("mem1 = malloc(64)\n");
    tags_print(print_object);

    int *mem2 = (int *) mallocing(32, print_status, false);
    printf("mem2 = malloc(32)\n");
    tags_print(print_object);

    freeing(mem1, 64, print_status, false);
    tags_print(print_object);
   ...
   // Check if realloc extended in place
    if (new_mem == mem2) {
        printf("\nPASS: Yes, realloc extended in place.\n\n");
    } else {
        printf("\nFAIL: No, realloc allocated a new block.\n");
    }
}
